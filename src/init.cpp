...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-addrindex", strprintf(_("Maintain an address index for looking up transactions by source or destination (default: %u)"), DEFAULT_ADDRINDEX));
...
bool AppInitParameterInteraction()
{
    ...
    if (GetArg("-prune", 0)) {
        if (GetBoolArg("-txindex", DEFAULT_TXINDEX) || GetBoolArg("-addrindex", DEFAULT_ADDRINDEX))
            return InitError(_("Prune mode is incompatible with -txindex and -addrindex."));
    }
